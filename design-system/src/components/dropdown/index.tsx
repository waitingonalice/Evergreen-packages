...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex cursor-default select-none items-center rounded-sm px-2 py-1.5 outline-none data-[state=open]:bg-accent",
      inset && "pl-8",
      className
    )}
    {...props}
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-gray-2 bg-white",
      inset && "pl-8",
      className
    )}
    {...props}
  ...
  (
    ...
    return (
      <DropdownMenuPrimitive.CheckboxItem
        ref={ref}
        className={cn(
          "relative flex cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-gray-2 bg-white",
          className
        )}
        checked={checked}
...
interface DropdownProps extends DropdownMenuPrimitive.DropdownMenuProps {
  label?: string;
  children: React.ReactNode;
  trigger: React.ReactNode;
}
function Dropdown(props: DropdownProps) {
  const { label, children, trigger, ...rest } = props;
  return (
    <DropdownMenu {...rest}>
